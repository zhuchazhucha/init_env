...
" Avoid reloading {{{1
if exists('loaded_gofolding')
    finish
endif

let loaded_gofolding = 1
" }}}

" .vimrc variable to disable autofolding for php files {{{1
if !exists("g:DisableAutoGOFolding")
    let g:DisableAutoGOFolding = 0
endif
" }}}
...
function! s:PHPCustomFolds() " {{{
	...
	call s:PHPFoldPureBlock('^func', s:FOLD_WITH_PHPDOC)
	call s:PHPFoldPureBlock('^type', s:FOLD_WITH_PHPDOC)
	call s:PHPFoldPureBlock('^var', s:FOLD_WITH_PHPDOC)

	" Fold includes with their PhpDoc
	call s:PHPFoldProperties('^\s*import', ")", s:FOLD_WITH_PHPDOC)
...
function! s:PHPFoldPureBlock(startPattern, ...) " {{{
	...
	while 1
		...
		if s:lineStart != 0
			...
			let s:lineStop = s:FindPureBlockEnd('{', '}', s:SEARCH_PAIR_START_FIRST, 1, "{$")
...
function! s:FindPureBlockEnd(startPair, endPair, searchStartPairFirst, ...) " {{{
	...
	if a:searchStartPairFirst == s:SEARCH_PAIR_START_FIRST
        if a:0 == 2
            let preStartPair = a:2
        else
            let preStartPair = a:startPair
        endif
		let line = search(preStartPair, 'W')
	endif
...
function! s:CheckAutocmdEnableGOFold()
    if &filetype == "go" && ! g:DisableAutoGOFolding
...
augroup SetGOFolds
    au!
    au BufReadPost * call s:CheckAutocmdEnableGOFold()
augroup END
" }}}
